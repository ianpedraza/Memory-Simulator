...
public class Kernel extends Thread
{
    ...
    try 
    {
      ...
      while ((line = in.readLine()) != null) 
      {
        if (line.startsWith("READ") || line.startsWith("WRITE")) 
        {
          ...
          {
            instructVector.addElement(new Instruction(command,Common.randomLong( address_limit ), Common.randomLong( address_limit ), Common.randomLong( address_limit )));
          } 
          else 
          { 
            if ( tmp.startsWith( "bin" ) )
            {
              ...
              initialSize = Long.parseLong(st.nextToken(),2);             
              finalSize = Long.parseLong(st.nextToken(),2);             
            }
            else if ( tmp.startsWith( "oct" ) )
            {
              addr = Long.parseLong(st.nextToken(),8);
              initialSize = Long.parseLong(st.nextToken(),8);             
              finalSize = Long.parseLong(st.nextToken(),8);  
            }
            else if ( tmp.startsWith( "hex" ) )
            {
              addr = Long.parseLong(st.nextToken(),16);
              initialSize = Long.parseLong(st.nextToken(),16);             
              finalSize = Long.parseLong(st.nextToken(),16);  
            }
            else
            {
              addr = Long.parseLong(tmp);
              initialSize = Long.parseLong(st.nextToken());             
              finalSize = Long.parseLong(st.nextToken());  
            }
            ...
            instructVector.addElement(new Instruction(command, addr, initialSize, finalSize));
          }
        }
      }
      in.close();
  ...
  public void step()
  {
    ...
    if ( instruct.inst.startsWith( "READ" ) ) 
    {
      ...
      if (segments == null || page.physical == -1 ) 
      {
        if ( doFileLog )
        {
